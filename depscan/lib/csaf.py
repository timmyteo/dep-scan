...
from vdb.lib.utils import compress_str
...
REF_MAP = {
    ...
    "https://github.com/[\w\d\-.]+/[\w\d\-.]+/security/advisories": "GitHub Advisory",
    ...
    "https://www.npmjs.com/package/@?\w+/?\w+": "NPM Package Page",
...
class CsafOccurence:
    def __init__(self, res):
        ...
        self.type = (res["type"],)
        self.pkg = (
            res["matched_by"].split("|")[1] + ":" + res["matched_by"].split("|")[2]
        )
...
def cleanup_results(results):
    ...
    for key, value in list(new_results.items()):
        if isinstance(value, dict):
            cleanup_results(value)
...
def format_cwe(vdata):
    ...
    """
    r = deepcopy(vdata)
    if len(r["problem_type"]) == 0 or r.get("problem_type") in ["UNKNOWN", [], "[]"]:
        r["cwe"] = None
        r["notes"] = []
        return r
    ...
    try:
        if CWE_MAP.get(fmt_cwe[0]):
            ...
            print(
                f"We couldn't locate the name of the CWE with the following id: {fmt_cwe[0]}. Help us out by reporting the "
                f"id at https://github.com/owasp-dep-scan/dep-scan/issues."
            )
        if len(fmt_cwe) > 1:
            ...
            rev_hx.append(
                {
                    ...
                    if tracking["initial_release_date"]
                    else str(get_date()),
                    "number": "1",
                    "summary": "Initial",
                }
            ...
            rev_hx.append(
                {
                    ...
                    if tracking["current_release_date"]
                    else str(get_date()),
...
def set_product_tree(tree):
    ...
    if not product_tree:
        LOG.info(
            "Unable to load product tree file. Please check your "
            "filepath in csaf.toml under product_tree.easy_import and "
            "that your product tree is a valid json file.\nProducing a "
            "CSAF without this component will cause schema validation to "
            "fail."
        )
    return product_tree
...
def parse_toml(metadata):
    ...
    """
    rev_hx = parse_revision_history(
        metadata["revision_history"]["revision"], metadata["tracking"]
    )
    dt = get_date()
    refs = []
    ...
    if not product_tree:
        raise Exception(
            "You must include a product_tree.full_product_name entry in your toml file."
        )
    output = {
        "document": {
            "aggregate_severity": {},
            ...
            "tracking": {
                ...
                "current_release_date": metadata["tracking"]["current_release_date"]
                or rev_hx[0]["date"]
                or dt,
                "initial_release_date": metadata["tracking"]["initial_release_date"]
                or rev_hx[-1]["date"]
                or dt,
                "revision_history": rev_hx,
                "status": metadata["tracking"]["status"],
                "generator": {
                    "date": metadata["tracking"]["current_release_date"]
                    or rev_hx[0]["date"]
                    or dt,
    ...
    if (
        ...
        raise Exception(
            "You cannot have an initial release date later than the current date. The most likely cause "
...
def export_csaf(results):
    ...
    json.dump(template_cleaned, open("../../contrib/sample.json", "w"), indent=4)
...
def import_csaf_toml():
    ...
    """
    with open("../../contrib/csaf.toml", "rb") as f:
        try:
            data = tomli.load(f)
