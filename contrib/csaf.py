...
class CsafOccurence:
    def __init__(self, res):
        ...
        self.cwe = {
            "id": res["problem_type"].replace("[", "").replace("]", ""),
            "name": "",
        }
        ...
        self.orig_date = (res["vdetails"]["source_orig_date"],)
        self.update_date = res["vdetails"]["source_update_date"]
...
def export_csaf_vuln(c):
    return {
        ...
        "scores": {"cvss_v3": c.cvss_v3, "products": [c.pkg]},
    }
...
def export_csaf(results):
    sample = {
        ...
        "product_tree": {"full_product_name": ""},
        "vulnerabilities": [],
    }
    # TODO: add import of config data and settings, creation of default if not provided
    # TODO: add automatic parsing of previous version's revision history if enabled and filepath provided, maybe store this in config?
    # TODO: autoincrement versions and revisions if enabled in config
    severity_ref = {
        "CRITICAL": 1,
        "HIGH": 2,
        "MEDIUM": 3,
    ...
    sample["document"]["aggregate_severity"]["text"] = agg_severity
    json.dump(sample, open("sample.json", "w"), ensure_ascii=False, indent=4)
